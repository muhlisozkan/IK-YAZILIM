...
function renderAnnualDashboard(data){
  ...
  $('#app').innerHTML=`<div class="section-title"><div><h2>${annualYear} yıllık izinleri</h2><span class="muted">Kullanım yalnızca puantajda Y (Yıllık İzin) girilen ve tarihi gelmiş günlerden hesaplanır</span></div>${leaveTabs()}</div>
    ...
    <div class="card" style="margin-top:18px"><div class="card-head"><div><h2>Çalışan yıllık izinleri</h2><span class="muted">Kalan = Toplam hak − kullanılan (çizelge + puantajdaki Y günleri + onaylı yıllık izin talepleri)${Number(data.not_entitled_count)>0?` · <strong>${data.not_entitled_count}</strong> çalışan henüz yıllık izin hak etmedi (listede yok)`:''}</span></div></div><div class="annual-leave-table"><table><thead><tr><th>ÇALIŞAN</th><th>DEPARTMAN</th><th>TOPLAM HAK EDİLEN</th><th>BU YIL HAK EDİLEN</th><th>KULLANILAN</th><th>KALAN</th><th></th></tr></thead><tbody>${tableRows||'<tr><td colspan="7" class="empty">Gösterilecek çalışan yok</td></tr>'}</tbody></table></div></div>`;
  bindLeaveTabs();
...
function leaveModal(){
  ...
  $('#l-start').onchange=()=>{applyEndLimits();renderWeekOff();preview();};
  $('#l-end').onchange=()=>{renderWeekOff();preview();};
