...
function editAnnualLeave(row){
  if(!row)return;
  modal(`${leaveEsc(row.employee_name)} · ${row.year} yıllık izin`, `<div class="form-grid"><div class="field"><label>Hak edilen gün</label><input class="input" id="annual-entitled" type="number" min="0" max="365" step="0.5" value="${row.entitled_days}"></div><div class="field"><label>Manuel düzeltme (+ / −)</label><input class="input" id="annual-adjustment" type="number" min="-365" max="365" step="0.5" value="${row.manual_adjustment}"></div><div class="field" style="grid-column:1/-1"><label>Düzeltme açıklaması</label><input class="input" id="annual-note" maxlength="500" value="${leaveEsc(row.adjustment_note||'')}"></div></div><div class="formula" style="margin-top:14px">Kullanılan ${leaveNumber(row.used_days)} gün puantajdan gelir ve burada değiştirilemez. Puantaj düzeltildiğinde bakiye otomatik güncellenir.</div>`,async()=>{
    const entitled=Number($('#annual-entitled').value),adjustment=Number($('#annual-adjustment').value);
    if(!Number.isFinite(entitled)||entitled<0||!Number.isFinite(adjustment))return toast('İzin değerlerini kontrol edin');
    try{await leaveApi(`/api/annual-leave-balances/${row.employee_id}`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({year:row.year,entitled_days:entitled,manual_adjustment:adjustment,adjustment_note:$('#annual-note').value.trim()})});closeModal();renderAnnualLeave();toast('Yıllık izin bakiyesi güncellendi')}catch(error){toast(error.message)}
  });
}
